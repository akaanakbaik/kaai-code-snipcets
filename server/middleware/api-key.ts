...
import { apiKeysTable, apiKeyUsageTable } from "../lib/schema.js";
...
export async function requireApiKey(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  if (!raw || typeof raw !== "string") {
    res.status(401).json({ error: "UNAUTHORIZED", message: "API key required. Pass X-API-Key header." });
    return;
  }
  ...
  try {
    ...
    if (dayCount >= apiKey.rateLimitPerDay) {
      res.status(429).json({ error: "RATE_LIMITED", message: `Daily limit reached: max ${apiKey.rateLimitPerDay} req/day`, retryAfter: 86400 });
      return;
    }

    if (monthCount >= apiKey.rateLimitPerMonth) {
      res.status(429).json({ error: "RATE_LIMITED", message: `Monthly limit reached`, retryAfter: 3600 * 24 * 7 });
      return;
    }
...
async function updateUsage(keyId: string, today: string, month: string): Promise<void> {
  ...
  if (usage) {
    await db
      ...
      .set({ requestsToday: usage.requestsToday + 1, requestsMonth: usage.requestsMonth + 1, updatedAt: new Date() })
