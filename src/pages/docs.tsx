...
export default function Docs() {
  return (
    <div className="max-w-3xl mx-auto py-8 space-y-10">
      ...
      <section>
        ...
        <CodeBlock>{`https://codes-snippet.kaai.my.id/api`}</CodeBlock>
  ...
  "https://codes-snippet.kaai.my.id/api/snippets?language=typescript&sortBy=latest&limit=5"`}</CodeBlock>
  ...
  "totalPages": 4
        ...
        <Card className="border-border/50 mb-4">
          ...
          <CardContent>
            <CodeBlock>{`curl -H "X-API-Key: kaai_your_key" \\
  https://codes-snippet.kaai.my.id/api/snippets/12345ABCDE`}</CodeBlock>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/50 mb-4">
          ...
          <CardContent>
            <CodeBlock>{`curl -H "X-API-Key: kaai_your_key" \\
  https://codes-snippet.kaai.my.id/api/snippets/popular`}</CodeBlock>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/50 mb-4">
          ...
          <CardContent>
            <CodeBlock>{`curl -H "X-API-Key: kaai_your_key" \\
  https://codes-snippet.kaai.my.id/api/snippets/tags`}</CodeBlock>
          </CardContent>
        </Card>
  ...
  "totalLanguages": 8
      ...
      {/* Locked Snippets */}
      <section>
        <SectionTitle icon={Shield} title="Locked Snippets" />
        <div className="space-y-3 text-sm text-muted-foreground">
          <p>
            Some snippets may be protected with a password or PIN. When a snippet is locked, the <code className="bg-zinc-900 px-1 rounded text-blue-300">code</code> field
            is omitted from the response. To access the code:
          </p>
          <p className="font-medium text-foreground/80">Step 1 — Unlock the snippet</p>
          <CodeBlock lang="http">{`POST https://codes-snippet.kaai.my.id/api/snippets/:id/unlock
Content-Type: application/json

{ "password": "your-password-or-pin" }`}</CodeBlock>
          <p>On success, you receive an unlock token (valid 1 hour):</p>
          <CodeBlock lang="json">{`{
  "success": true,
  "token": "...",
  "expiresIn": 3600
}`}</CodeBlock>
          <p className="font-medium text-foreground/80">Step 2 — Fetch the snippet with the token</p>
          <CodeBlock lang="http">{`GET https://codes-snippet.kaai.my.id/api/snippets/:id
X-Unlock-Token: <token>`}</CodeBlock>
          <p className="text-xs text-muted-foreground/70">Rate limit: 5 wrong attempts per 15 minutes (exponential backoff).</p>
        </div>
      </section>
