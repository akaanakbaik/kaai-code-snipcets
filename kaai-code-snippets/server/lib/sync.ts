...
// Backup repo — user's dedicated backup repository
...
async function backupToGithub(): Promise<void> {
  ...
  try {
    ...
    // Backup all snippets (full dump) to primary source repo
    ...
    // Backup approved snippets to dedicated backup repo (aka-second/code-snipset)
    if (BACKUP_GITHUB_TOKEN && BACKUP_GITHUB_REPO) {
      // Full backup JSON
      const backupContent = Buffer.from(JSON.stringify({
        exportedAt: now.toISOString(),
        totalSnippets: snippets.length,
        approvedSnippets: approved.length,
        snippets: approved,
      ...
      await pushToGithub(
        BACKUP_GITHUB_TOKEN,
        BACKUP_GITHUB_REPO,
        `backup/snippets-${dateStr}.json`,
        backupContent,
        `[Auto] Backup ${approved.length} approved snippets — ${dateStr}`,
      ).catch((err) => {
        logger.warn(`[sync] Backup repo push failed: ${(err as Error).message}`);
      });

      // Also maintain an always-current "latest.json" file
      const latestContent = Buffer.from(JSON.stringify({
        exportedAt: now.toISOString(),
        totalSnippets: snippets.length,
        approvedSnippets: approved.length,
        snippets: approved,
      ...
      await pushToGithub(
        ...
        `backup/latest.json`,
        latestContent,
        `[Auto] Update latest.json — ${timestamp}`,
      ).catch((err) => {
        logger.warn(`[sync] Backup repo latest.json failed: ${(err as Error).message}`);
      });

      logger.info(`[sync] Backup repo updated: ${approved.length} approved snippets → ${BACKUP_GITHUB_REPO}/backup/`);
    }
  } catch (err) {
